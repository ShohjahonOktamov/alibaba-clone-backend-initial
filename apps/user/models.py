...
class User(AbstractBaseUser, PermissionsMixin, Model):
    ...
    phone_number: CharField = CharField(max_length=13, unique=True)
    email: CharField = CharField(max_length=255, unique=True)
    password: CharField = CharField(max_length=255)
    first_name: CharField = CharField(max_length=255)
    last_name: CharField = CharField(max_length=255)
    gender: CharField = CharField(max_length=10)
    groups: ManyToManyField = ManyToManyField(to="Group", related_name="custom_user_groups")
    policies: ManyToManyField = ManyToManyField(to="Policy")
    ...
    date_joined: DateTimeField = DateTimeField(auto_now_add=True, null=True)
    last_login: DateTimeField = DateTimeField(auto_now=True, null=True)
...
class Policy(Model):
    ...
    id: UUIDField = UUIDField(primary_key=True, default=uuid4)
    name: CharField = CharField(max_length=13, unique=True, choices=policy_choices)
    permissions: ManyToManyField = ManyToManyField(to=Permission)
    is_active: BooleanField = BooleanField(default=False)
    created_by: ForeignKey = ForeignKey(to=User, null=True, on_delete=CASCADE)
    created_at: DateTimeField = DateTimeField(auto_now_add=True)
    updated_at: DateTimeField = DateTimeField(auto_now=True)
...
class Group(Model):
    ...
    policies: ManyToManyField = ManyToManyField(to=Policy)
    permissions: ManyToManyField = ManyToManyField(to=Permission, related_name="custom_group_permissions")
    is_active: BooleanField = BooleanField(default=False)
    created_by: ForeignKey = ForeignKey(to=User, null=True, on_delete=CASCADE)
    created_at: DateTimeField = DateTimeField(auto_now_add=True)
    updated_at: DateTimeField = DateTimeField(auto_now=True)
...
class SellerUser(Model):
    ...
    company: CharField = CharField(max_length=50, null=True)
    image: BinaryField = BinaryField(null=True)
    bio: CharField = CharField(max_length=255, null=True)
    birth_date: DateTimeField = DateTimeField(null=True)
    country: CharField = CharField(max_length=50, null=True)
    city: CharField = CharField(max_length=50, null=True)
    district: CharField = CharField(max_length=50, null=True)
    street_address: CharField = CharField(max_length=50, null=True)
    postal_code: CharField = CharField(max_length=10, null=True)
    second_phone_number: CharField = CharField(max_length=13, null=True)
    building_number: CharField = CharField(max_length=50, null=True)
